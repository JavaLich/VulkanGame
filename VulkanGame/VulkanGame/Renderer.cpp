...
void Renderer::createDepthResources() {
	...
	createImage(swapChainExtent.width, swapChainExtent.height, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VMA_MEMORY_USAGE_GPU_ONLY, depthImage, depthImageMemory, depthImageAllocInfo);
...
void Renderer::createTextureImage() {
	...
	stbi_uc* pixels = stbi_load("assets/textures/mona-lisa.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	...
	createBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, stagingBuffer, stagingBufferMemory, VMA_MEMORY_USAGE_CPU_ONLY, stagingAllocInfo);
...
void Renderer::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VmaMemoryUsage memoryUsage, VkImage & image, VmaAllocation & imageMemory, VmaAllocationInfo& allocInfo)
{
	...
	imageAllocInfo.flags = VMA_ALLOCATION_CREATE_MAPPED_BIT;
	if (vmaCreateImage(allocator, &imageInfo, &imageAllocInfo, &image, &imageMemory, &allocInfo) != VK_SUCCESS) {
...
void Renderer::createVertexBuffers() {
	...
	if (bufferSize + indexBufferSize%minUniformBufferOffsetAlignment == 0)fullBufferSize = bufferSize + indexBufferSize + sizeof(UniformBufferObject);
	else fullBufferSize = bufferSize + indexBufferSize + (256+(minUniformBufferOffsetAlignment-(bufferSize+indexBufferSize)%minUniformBufferOffsetAlignment)) + sizeof(UniformBufferObject);
...
void Renderer::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkBuffer & buffer, VmaAllocation & bufferMemory, VmaMemoryUsage memoryUsage, VmaAllocationInfo& allocInfo)
{
	...
	if (vmaCreateBuffer(allocator, &bufferInfo, &allocCreateInfo, &buffer, &bufferMemory, &allocInfo) != VK_SUCCESS) {
...
int Renderer::rateDeviceSuitability(VkPhysicalDevice device) {
	...
	//If device lacks one of these then it is not suitable
	if (!deviceFeatures.geometryShader || !findQueueFamilies(device).isComplete() || !extensionsSupported || !swapChainAdequate||!deviceFeatures.samplerAnisotropy) {
		return 0;
	}
...
void Renderer::createGraphicsPipeline() {
	auto vertShaderCode = Helper::readFile("assets/shaders/vert.spv");
	auto fragShaderCode = Helper::readFile("assets/shaders/frag.spv");
	...
	rasterizer.depthBiasEnable = VK_FALSE; // If enabled configure other depth bias values in rasterizer

	VkPipelineMultisampleStateCreateInfo multisampling = {};
	multisampling.sType = VK_STRUCTURE_TYPE_PIPELINE_MULTISAMPLE_STATE_CREATE_INFO;
	multisampling.sampleShadingEnable = VK_FALSE;//Need to enable configure sampleshading and alpha to one values 
	...
	//Specify Dynamic States if needed(VkDynamicState)
...
void Renderer::checkRequiredExtensionsPresent(std::vector<VkExtensionProperties> availableExt, const char** requiredExt, int requiredExtCount) {
